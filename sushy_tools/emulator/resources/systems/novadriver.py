...
class OpenStackDriver(AbstractSystemsDriver):
    ...
    def systems(self):
        ...
        """
        return [server.name for server in self._cc.list_servers()]
    ...
    def get_power_state(self, identity):
        ...
        try:
            instance = self._vm
            self._logger.debug(instance.name)
            self._logger.debug(instance.status)
            self._logger.debug(instance.task_state)
            if instance.status in ['STARTED','ACTIVE','BUILDING', 'RESCUE']:
                if instance.task_state in ["powering-off","unrescuing"]:
                  self._logger.debug('returning power state OFF for vm ' + instance.name)
                  return  "Off"
                if instance.task_state == "stopped":
                  self._logger.debug('returning power state OFF for vm ' + instance.name)
                  return "Off" 
                if instance.status == "ACTIVE" and instance.name in self._server_unrescued:
                  self._logger.debug('auto stopping ' + instance.name)
                  self._cc.compute.stop_server(self._vm)
                  del self._server_unrescued[instance.name]
                  self._logger.debug('returning power state OFF for vm ' + instance.name)
                  return "Off"
                self._logger.debug('returning power state ON for vm ' + instance.name)
                return "On"
#            else:
#                if instance.name in self._boot_order_changed:
#                  self._logger.debug('boot order changed for vm ' + instance.name + ' to ' + self._boot_order_changed[instance.name])
#                  self._cc.compute.set_server_metadata(instance.id,bootorder=self._boot_order_changed[instance.name])
#                  metadata = self._cc.compute.get_server_metadata(instance.id)
#                  if self._boot_order_changed[instance.name] == "network":
#                    self._logger.debug('autostart (rescue) ' + instance.name + ' using ' + self._boot_order_changed[instance.name])
#                    self._cc.compute.rescue_server(instance.id,image_ref=self._pxe_image)
#                  else:
#                    self._logger.debug('autostart (unrescue) ' + instance.name + ' using ' + self._boot_order_changed[instance.name])
#                    try:
#                        self._cc.compute.unrescue_server(instance.id)
#                    except:
#                        pass
#                    self._cc.compute.start_server(instance.id)
#                  del self._boot_order_changed[instance.name]
                self._logger.debug('returning power state OFF for vm ' + instance.name)
                return "Off"

        except Exception as e:
            logging.error(instance.name + ' get_power_state:' + str(e))
            if instance.status in ['STARTED','ACTIVE','BUILDING', 'RESCUE']:
                self._logger.debug('returning power state ON for vm ' + instance.name)
                return "On"
            self._logger.debug('returning power state OFF for vm ' + instance.name)
        ...
        if state in ('On', 'ForceOn'):
           if instance.status == 'SHUTOFF':
              try:
                  ...
                  if current == "network":
                   self._logger.debug(self._pxe_image)
                   self._cc.compute.rescue_server(instance.id,image_ref=self._pxe_image)
                  else:
                   self._cc.compute.start_server(instance.id)
              except Exception as e:
                  logging.error(instance.name + ' power_on:' + str(e))
                  return 0xc0
           else:
             return False
           ...
           if instance.status in ['STARTED','ACTIVE']:
                try:
                    self._cc.compute.stop_server(instance.id)
                except Exception as e:
                    logging.error(instance.name + ' power_off:' + str(e))
                    return 0xc0
           elif instance.status in ['RESCUE']:
                try:
                    self._cc.compute.unrescue_server(instance.id)
                    self._server_unrescued[instance.name] = True
                except Exception as e:
                    logging.error(instance.name + ' power_off:' + str(e))
                    return 0xc0
    ...
    def get_boot_device(self, identity):
        ...
        if "bootorder" in metadata:
           return metadata.metadata["bootorder"]
        return 0x04
    ...
    def set_boot_device(self, identity, boot_source):
        ...
        self._logger.debug('set boot device ' + target + ' for vm ' + instance.name + ' with status ' + instance.status)
        metadata = self._cc.compute.get_server_metadata(instance.id)
        current = ""
        if "bootorder" in metadata.metadata:
          current = metadata.metadata["bootorder"]
        if current != target :
          if target in ("hd"):
            if instance.status == "RESCUE":
              self._cc.compute.unrescue_server(instance.id)
              self._server_unrescued[instance.name] = True
          elif target in ("network"):
              self._cc.compute.rescue_server(instance.id,image_ref=self._pxe_image)
          #self._boot_order_changed[instance.name] = target 
        return True
