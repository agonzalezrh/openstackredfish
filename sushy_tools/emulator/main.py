...
class Application(flask.Flask):
    ...
    def systems(self):
        ...
        pxe_image = self.config.get('SUSHY_EMULATOR_PXE_IMAGE')
        if os_cloud:
            ...
            result = novadriver.OpenStackDriver.initialize(
                self.config, self.logger, os_cloud,pxe_image=pxe_image)()
...
def parse_args():
    ...
    parser.add_argument('--pxe-image',
                               type=str,
                               help='OpenStack pxe image to boot on rescue mode'
                               )
...
def main():
    ...
    if args.pxe_image:
        app.config['SUSHY_EMULATOR_PXE_IMAGE'] = args.pxe_image
